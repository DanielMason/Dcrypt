...
int main(int argc, char** argv) {
  ...
  step1_frame = gtk_frame_new("Step 1. Select file to Encrypt:");
  gtk_frame_set_shadow_type(GTK_FRAME(step1_frame), GTK_SHADOW_ETCHED_IN);


  select_file_label = gtk_label_new("Step 1. Select file to Encrypt:");
  gtk_fixed_put(GTK_FIXED(step1_frame), select_file_label, 10, 10); 

  select_file_button = gtk_button_new_with_label("Select File");
  gtk_fixed_put(GTK_FIXED(step1_frame), select_file_button, 10, 30);
